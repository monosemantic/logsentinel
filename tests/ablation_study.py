...
def normalize_state(state):
    ...
    if "WARN" in state:
        return "WARNING"
    if "RECOVERY" in state or "INFO" in state:
        return "INFO"
    return "INFO"
...
def compute_metrics(y_true, y_pred):
    ...
    fp_count = sum(
        1 for t, p in zip(y_true, y_pred) if t != "CRITICAL" and p == "CRITICAL"
    )
...
def classify_regex(raw_log):
    ...
    if any(k in log for k in ["critical", "fatal", "emergency", "panic"]):
        ...
        k in log for k in ["error", "exception", "traceback", "failed", "failure"]
    ):
        return "ERROR"
...
for idx, log in enumerate(dataset):
    source_id = f"{log.get('source_id', 'test')}-abl{log['id']}"
...
if empty_count > 0:
    print(
        f" {empty_count}/{len(dataset)} réponses vides — vérifier les nœuds email dans le workflow."
    )
...
print("=" * 75)
print(
    f"{'Système':<25} {'Accuracy':>10} {'F1 Macro':>10} {'Faux Pos':>10} {'Latence':>12}"
)
print(f"{'-'*25} {'-'*10} {'-'*10} {'-'*10} {'-'*12}")
print(
    f"{'1 — Regex Brut':<25} {acc_s1:>9.1%} {f1_s1:>10.2f} {fp_s1:>9.1%} {lat_s1:>10.1f}ms"
)
print(
    f"{'2 — AFD seul':<25} {acc_s2:>9.1%} {f1_s2:>10.2f} {fp_s2:>9.1%} {lat_s2:>10.1f}ms"
)
print(
    f"{'3 — AFD + LLM':<25} {acc_s3:>9.1%} {f1_s3:>10.2f} {fp_s3:>9.1%} {lat_s3:>10.1f}ms"
)
print("=" * 75)
...
for i, (true, r_pred, a_pred, msg) in enumerate(failures_regex[:3], 1):
    print(
        f'  {i}. Attendu: {true} | Regex: {r_pred} (Échec) | AFD: {a_pred} (Succès)\n     Log: "{msg[:120]}"'
    )
...
failures_afd = [
    (t, a, log["raw_log"]) for log, t, a in zip(dataset, y_true_s2, y_pred_s2) if a != t
]
for i, (true, a_pred, msg) in enumerate(failures_afd[:3], 1):
    print(
        f'  {i}. Attendu: {true} | AFD Seul: {a_pred} (Échec)\n     Log: "{msg[:120]}"'
    )
if not failures_afd:
    print("  Aucun cas trouvé dans ce dataset.")
...
report_out = {
    "systems": {
        "regex_brut": {
            "accuracy": acc_s1,
            "macro_f1": f1_s1,
            "false_positive_rate": fp_s1,
            "latency_ms": lat_s1,
        },
        "afd_seul": {
            "accuracy": acc_s2,
            "macro_f1": f1_s2,
            "false_positive_rate": fp_s2,
            "latency_ms": lat_s2,
        },
        "afd_plus_llm": {
            "accuracy": acc_s3,
            "macro_f1": f1_s3,
            "false_positive_rate": fp_s3,
            "latency_ms": lat_s3,
        },
    }
}
